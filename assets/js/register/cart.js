jQuery(document).ready(function ($) {
    ...
    window.POS_CART = {
        ...
        addToCart: function (product_data, product_id, quantity, variation_id, variation, cart_item_data) {
            ...
            try {
                ...
                if (variation_data) {
                    ...
                    if (variation_data.manage_stock === true) {
                        var managing_stock = variation_data.manage_stock;
                         ...
                         */
                        console.log(check_qty + quantity);
                    ...
                    // Stock check - this time accounting for whats already in-cart
                    if (product_data.manage_stock === true) {
                        var managing_stock = product_data.manage_stock;
                        var products_qty_in_cart = CART.get_cart_item_quantities();
        ...
        has_enough_stock: function (product_data, quantity) {
            ...
            var has_enough_stock = false;

            if(product_data.sold_individually === true && quantity > 1)
                return false;

            if(product_data.manage_stock === false)
                return true;

            if(product_data.stock_quantity >= quantity)
                return true;

            if(product_data.backorders_allowed === true)
                return true;
        ...
        calculate_totals: function () {
            ...
            $.each(cart, function (cart_item_key, values) {
                ...
                var line_price = parseFloat(_product.regular_price) <= parseFloat(values["price"]) ?_product.regular_price * values['quantity'] : values["price"] * values['quantity'];
                ...
                if (_product.tax_status !== "taxable" || wc_pos_params.pos_calc_taxes === false) {
                    ...
                    var _line_price = values['original_price'] ? values['original_price'] : line_price;
                    CART.subtotal += _line_price;
                    CART.subtotal_ex_tax += _line_price;
                    ...
                    if (item_tax_rates !== base_tax_rates) {
                        ...
                        var taxes = TAX.calc_tax(_line_price, base_tax_rates, true, true);

                        // Now we have a new item price (excluding TAX)
                        line_subtotal = _line_price - array_sum(taxes);
                        ...
                        // Calc tax normally
                        var taxes = TAX.calc_tax(_line_price, item_tax_rates, true);
                    ...
                    line_subtotal_tax = array_sum(taxes);
                    // We check `original_price` property as loaded orders has `original_price`
                    line_subtotal = values['original_price'] ? values['original_price'] : line_price;
                }
            ...
            $.each(cart, function (cart_item_key, values) {
                ...
                var line_price = _product.regular_price * values['quantity'];
                ...
                if (_product.tax_status !== "taxable" || wc_pos_params.pos_calc_taxes === false) {
                    ...
                    var discounted_price = base_price;
                    var _base_price = parseInt(POS_TRANSIENT.order_id) > 0 ? _product.regular_price : base_price;
                    var _discounted_price = CART.get_discounted_price(values, _base_price, true);
                    if(!POS_TRANSIENT.order_id || !(parseInt(POS_TRANSIENT.order_id) > 0)){
                ...
                CART.cart_contents[cart_item_key]['taxes'] = [];
                $.each(discounted_taxes, function (i, tax) {
                    CART.cart_contents[cart_item_key]['taxes'].push({
                        total: parseFloat(tax).toFixed(2),
                        subtotal: parseFloat(taxes[i]).toFixed(2)
                    });
                });
            ...
            var r = accounting.parse(accounting.toFixed(CART.cart_contents_total + CART.tax_total + CART.shipping_total + CART.fee_total, CART.dp));
            CART.total = r;
            ...
            $('.item_count .count').text(Math.round(total_items * 100) / 100);
        },
        /**
         * Sort by subtotal
         * @param  array $a
            ...
            if (Object.size(CART.coupons) > 0) {
                ...
                $.each(CART.coupons, function (code, coupon) {
                    if (coupon.is_valid() && (coupon.is_valid_for_product(values['data'], values) || coupon.is_valid_for_cart() )) {
                        if (!coupon.is_valid_for_product(values['data']) && code.toLowerCase() !== 'pos discount') {
                            return price;
                        }
                        var discount_amount = coupon.get_discount_amount(price, values, true);
        ...
        increase_coupon_discount_amount: function (code, amount, tax) {
            ...
            if (code.toLowerCase() == 'pos discount') {
        ...
        get_coupon_discount_amount: function (code, ex_tax) {
            ...
            var discount_amount = isset(CART.coupon_discount_amounts[code]) ? CART.coupon_discount_amounts[code] : 0;
        ...
        setup_refund: function(refund_data){
            ...
            APP.db.get('orders', $this.pos_refund.order.id).done(function (order) {
                APP.loadOrder(order.id, $this.pos_refund.items);
