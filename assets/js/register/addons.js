jQuery(document).ready(function ($) {
    ...
    window.POS_ADDONS = {
        ...
        processPayment: function (cart, payment_method) {
            switch (payment_method) {
                ...
                case 'stripe':
                    ...
                    $( document ).on('stripeError', function(e, result){
                        toastr.clear();
                        toastr.remove();
                        APP.showNotice(result.error.message, 'error');
                        $('.blockUI').remove();
                        return false;
                    });
        ...
        goodCardScan: function (cardData) {
            if(typeof cardData.type != "undefined" && cardData.type == "generic"){
                var line = '';
                if(!empty(cardData.line1)){
                    line = cardData.line1;
                }else if(!empty(cardData.line2)){
                    line = cardData.line2;
                }else if(!empty(cardData.line3)){
                    line = cardData.line3;
                }
                cardData = ADDONS.cardParser(line);
            }
            var payment_method = $('.select_payment_method:checked:not(:disabled)').val(),
                exp_year = cardData.s_exp_year ? cardData.s_exp_year : cardData.exp_year;

            if(exp_year.length > 2) exp_year = exp_year.slice(-2);
            ...
            switch (payment_method) {
                ...
                case 'authorize_net_cim_credit_card':
                    ...
                    $('#wc-authorize-net-cim-credit-card-expiry').val(cardData.exp_month + '/' + exp_year);
