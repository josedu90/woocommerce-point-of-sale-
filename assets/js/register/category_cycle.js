(function ($) {
    ...
    function get_filter(elems, parent, title, offset, limit, scrolled) {
        ...
        if (pos_grid.grid_id != 'all' && pos_grid.grid_id != 'categories') {
            ...
            var search_pr = [];
            APP.db.values('products', pos_grid.products_sort).done(function (products) {
            ...
            if (!scrolled) {
                ul.find('.title_product').hide();
                updateVisibility(parent);
            }
            if (search_pr.length > 0 && false) { //TODO: Need check without this code
    ...
    function build_filter_variantion(elems, parent, offset, limit, pagination) {
        ...
        var ul = $('#grid_layout_cycle ul');
        elems.hide();
        $('.add_grid_tile, .open_variantion').hide();
        $('.open_variantion_attr, #tile_go_back, .product-loader').remove();
     ...
     */
    function updateVisibility(parentCategoryId) {
        $(".title_product").each(function () {
            let productCategories = $(this).data('root-category');
            if($.inArray(parentCategoryId, productCategories) !== -1){
                $(this).show();
            }
        });
    }
