...
(function ($) {
    ...
    WC_Coupon = function (code, data) {
        ...
        populate = function (data) {
            ...
            defaults.type = data.discount_type ? data.discount_type : data.type;
        ...
        function validate_cart_excluded_product_ids() {
            // Exclude Products
            if (sizeof(instance.excluded_product_ids) > 0) {
                var valid_for_cart = true;
                if (!CART.is_empty()) {
                    var cart = CART.get_cart();
                    $.each(cart, function (cart_item_key, cart_item) {
                        if (in_array(cart_item['product_id'], instance.excluded_product_ids) || in_array(cart_item['variation_id'], instance.excluded_product_ids)) {
        ...
        function validate_cart_excluded_product_categories() {
            if (sizeof(instance.excluded_product_categories) > 0) {
                var valid_for_cart = false;
                if (!CART.is_empty()) {
                    var cart = CART.get_cart();
                    $.each(cart, function (cart_item_key, cart_item) {
                        ...
                        var product_cats = cart_item.data.categories_ids;
                        // using vanilla javascript `for` because of dynamic array
                        for(var i = 0; i < product_cats.length; i++){
                            var product_cat = product_cats[i];
                            var cat = pos_grid.categories['_' + product_cat];
                            if(cat && cat.parent > 0){
                                product_cats.push(cat.parent);
                            }
                        }
                        if (product_cats && sizeof(array_intersect(product_cats, instance.excluded_product_categories)) === 0) {
                            valid_for_cart = true;
                        }
                    });
                }
        ...
        this.is_valid_for_product = function (product, values) {
            ...
            if (sizeof(instance.excluded_product_ids) > 0) {
                if (in_array(product.id, instance.excluded_product_ids) || ( ( product.variation_id ) && in_array(product.variation_id, instance.excluded_product_ids) )) {
                    valid = false;
                }
            }
                ...
                case E_WC_COUPON_EXCLUDED_CATEGORIES:
                    ...
                    if (!CART.is_empty()) {
                        ...
                        $.each(cart, function (cart_item_key, cart_item) {
                            ...
                            var product_cats = cart_item.data.categories_ids;
                            if (product_cats) {
                                var intersect = array_intersect(product_cats, instance.excluded_product_categories)
