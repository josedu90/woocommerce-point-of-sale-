jQuery(document).ready(function ($) {
    window.POS_PR_ADDONS = {
            ...
            if ($('#product-addons-attributes table').length) {
                ...
                $('#product-addons-attributes table').find('.addon').each(function () {
                    ...
                    if ($(this).is('.addon-custom-price')) {
                        ...
                        var addonData = $(this).data('addon');
                        var value = 0;
                        if(addonData.price_type == 'flat_fee'){
                            value = addonData.price;
                        }else if(addonData.price_type == 'quantity_based'){
                            value = parseFloat(addonData.price) * parseInt($(this).val());
                        }else if(addonData.price_type == 'percentage_based'){
                            if(product_price > 0){
                                var val = (product_price * parseInt($(this).val())) / 100 * parseFloat(addonData.price);
                                value = val;
                            }
                        }
                        addon_cost = value;
                        addon_cost_raw = value;
                    ...
                    $.each(product_addons, function (index, addon) {
                        ...
                        switch (addon.type) {
                            ...
                            case 'custom_textarea':
                                var $input = $('.' + addon.type + '_' + index + '_0');
                                if(!empty($input.val())){
                                    fields[0] = $input.val();
                                }
                                break;
        ...
        get_product_addons: function (adding_to_cart) {
            var product_addons = [];
            if (typeof adding_to_cart.post_meta != 'undefined' && typeof adding_to_cart.post_meta.product_addons != 'undefined') {
                var _product_addons = $.isArray(adding_to_cart.post_meta.product_addons) ? adding_to_cart.post_meta.product_addons : Object.values(adding_to_cart.post_meta.product_addons);
                product_addons = _product_addons.slice(0);
                    ...
                    $.each(addon, function (i, value) {
                        ...
                        switch (type) {
                            ...
                            case 'custom_email':
                                ...
                                break;
                            case 'input_multiplier':
                                var price_type = product_addons[index]['price_type'];
                                if(price_type == 'flat_fee'){
                                    option["price"] = product_addons[index]['price'];
                                }else if(price_type == 'quantity_based'){
                                    option["price"] = parseFloat(product_addons[index]['price']) * parseInt(value);
                                }else if(price_type == 'percentage_based'){
                                    if(product_data.price > cart_item_data.addons_price){
                                        var val = ((parseFloat(product_data.price) - cart_item_data.addons_price) * parseInt(value)) / 100 * parseFloat(product_addons[index]['price']);
                                        option["price"] = val;
                                    }
                                }
                        }
