...
#	        add_action( 'wp_login', array( $this, 'set_last_login') );
    ...
    public function init_addons_hooks()
    {
        ...
        if (class_exists('WC_Product_Addons') && get_option('wc_pos_force_enable_addons', 'no') == "yes") {
    ...
    public function is_pos_referer()
    {
        ...
        if(!$referer){
            if(isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'], 'point-of-sale') !== false){
                return true;
            };
        }
    ...
    public function wc_api_loaded()
    {
        ...
        include_once('api/class-wc-pos-api-products.php');
...
//            echo "\n" . '<script src="' . $script . '?v=' . $v . '"></script>';
    ...
    public static function getJsParams()
    {
        ...
        $params = apply_filters('wc_pos_params', array(
            ...
            'default_country' => get_option('wc_pos_default_country', 'GB'),
            'currency_format_num_decimals' => absint(get_option('woocommerce_price_num_decimals')),
            'currency_format_symbol' => get_woocommerce_currency_symbol(),
            'currency_format_decimal_sep' => esc_attr(stripslashes(get_option('woocommerce_price_decimal_sep'))),
            'currency_format_thousand_sep' => esc_attr(stripslashes(get_option('woocommerce_price_thousand_sep'))),
            'guest_checkout' => (get_option('wc_pos_guest_checkout', 'yes') == 'yes' ? true : false),
...
//            'wc_api_url' => home_url('wp-json/wc/v2/'),
            ...
            'offline_mode' => get_option('wc_pos_enable_offline', 'no') === "yes" ? true : false,
            'customer_saving' => get_option('wc_pos_customer_saving', 'no'),
            'customer_required_fields' => array_unique(array_merge(get_option('wc_pos_customer_create_required_fields', array(
                'billing_first_name',
                'billing_last_name',
                'billing_email',
                'billing_address_1',
    ...
    public static function getJsCartParams()
    {
        ...
        $params = apply_filters('wc_pos_cart_params', array(
            'prices_include_tax' => wc_prices_include_tax(),
            'calc_shipping' => (get_option('woocommerce_ship_to_countries') != 'disabled') ? true : false,
...
//        WC()->product_factory->get_product();
    ...
    public function getGrid($is_api = false)
    {
        ...
        if ($grid_id != 'all' && $grid_id != 'categories') {
            ...
            $grid = wc_point_of_sale_get_grid_by_id($grid_id);
            if($grid->auto_tiles_view == 'categories'){
                $grid_id = 'categories_auto';
            }
            if (count($tiles)) {
                $includedCategories = [];
                foreach ($tiles as $key => $style) {
                    ...
                    if($style->style == "auto"){
                        ...
                        if($grid_id == 'categories_auto' && !in_array($cat, $includedCategories)){
                            $includedCategories[] = $cat->term_id;
                        }
                        $term_ids = $wpdb->get_col("SELECT * FROM {$wpdb->prefix}term_taxonomy WHERE term_id = {$cat->term_id} or parent = {$cat->term_id}");
                        $product_ids = $wpdb->get_col("SELECT DISTINCT object_id FROM {$wpdb->prefix}term_relationships WHERE term_taxonomy_id IN (" .implode(',', $term_ids).")");
        ...
        if ($terms) {
            ...
            foreach ($terms as $term) {
                if($grid_id == 'categories_auto' && !in_array($term->term_id, $includedCategories)){
                    continue;
                }
        ...
        $grid = array(
            ...
            'grid_id' => $grid_id == 'categories_auto' ? 'categories' : $grid_id,
            'term_relationships' => $term_relationships,
            'categories' => $categories,
            'products_sort' => array(),
            'tile_styles' => $tile_styles,
            'tile_sorting' => get_option('wc_pos_default_tile_orderby', 'menu_order'),
            'auto_tiles' => $grid_id == 'categories_auto' ? 1 : 0
        );

        $product_limit = get_option('wc_pos_product_limit', 'no');
        if ('no' === $product_limit) {
            global $wpdb;
            $products_count = $wpdb->get_var("SELECT COUNT(*) FROM $wpdb->posts WHERE post_type='product'");
            $grid['tile_limit'] = $products_count;
        } else {
            $limit = get_option('wc_pos_product_limit_value', 100);
            $grid['tile_limit'] = is_numeric($limit) ? $limit : 100;
        }
