...
class WC_Pos_Cloud_Print_Job
{
    ...
    public function printjob($copy_count=1)
    {
        $fh = fopen($this->tempFilePath, 'w');
        fwrite($fh, hex2bin($this->printJobBuilder.self::SLM_FEED_PARTIAL_CUT_HEX));
        fclose($fh);
        WC_POS_CPI()->star_cloudprnt_queue_add_print_job($this->printerMac, $this->tempFilePath, $copy_count);
