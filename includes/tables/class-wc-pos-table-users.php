...
class WC_Pos_Table_Users extends WP_List_Table {
  ...
  function column_name($item){
      ...
      if ( current_user_can( 'edit_user',  $item['ID'] ) ) {
          $username = '<a href="user-edit.php?user_id='.$item['ID'].'" target="_blank">'.$item['username'].'</a>';
      }
   ...
   function column_outlet( $item ) {
      $outlet_arr = WC_POS()->outlet()->get_data_names($item['outlet']);
       if(!empty($item['outlet']) && count($item['outlet'])){
           $outlets_string = "";
           foreach ($item['outlet'] as $outlet){
               $outlets_string .= '<a href="admin.php?page=wc_pos_outlets&action=edit&id='.$outlet.'" target="_blank">'.$outlet_arr[$outlet].'</a><br/>';
           }
           return $outlets_string;
       }else{
           return '-';
       }
  }
    ...
    function column_orders( $item ) {
        ...
        return '<strong>' . wc_price($sales) . '</strong><br/>' . $item['orders'];
    }
  ...
  function get_columns() {
    $columns = array(
      'name'       => __( 'Cashier', 'wc_point_of_sale' ),
