...
class WC_Pos_Table_Tiles extends WP_List_Table
{
    ...
    function column_product($item)
    {
        ...
        if($item['style'] == "auto"){
            $term = get_term($item['product_id']);
            $name = $term ? $term->name : "";
            return sprintf('<strong><a href="?page=%4$s&grid_id=%5$s&action=%6$s&id=%7$s">%1$s</a></strong> %2$s %3$s',
                $name,
                '<p class="description">' . __("Tiles will be dynamic and automatically load from this category.", "wc_point_of_sale") . '</p>',
