...
class WC_POS_AJAX
{
    ...
    public function __construct()
    {
        ...
        $ajax_events = array(
            ...
            'reset_pos_settings' => false,
            'generate_order_id' => false,
            'find_products' => false,
            'scan_product' => false
        );
...
// Store variation data in meta
    ...
    public function get_product_by() {
        ...
        $url = WC_POS()->wc_api_url();
        if (is_null($by) || $by === 'id') {
            $url .= 'products/' . $find;
        } elseif ($by === 'sku') {
            $url .= 'products?' . http_build_query(array('sku' => $find));
        } else {
            wp_send_json_error();
        }

        // Allow REST API filtering before doing the request.
        ...
        if ($by === 'sku' && empty($data)) {
            wp_send_json_error([
                'code' => 'wc_pos_rest_product_sku_not_found',
                'data' => [
                    'status' => 404,
                ],
                'message' => __('SKU not found', 'wc_point_of_sale')
            ]);
        } elseif ( $by === 'sku' ) {
            $data = $data[0];
        }
    ...
    public function reset_pos_settings()
    {
        global $wpdb;
        ...
        foreach ($settings as $setting){
            ...
            foreach ($_settings as $_setting){
                if(in_array($_setting['type'], array('title', 'sectionend', 'button'))){
                    continue;
                }
        ...
        $result = $wpdb->query("DELETE FROM {$wpdb->options} WHERE option_name IN ('" . implode("', '", $in) . "')");
    ...
    public function generate_order_id()
    {
        $register_id = isset($_POST['register_id']) ? $_POST['register_id'] : 0;
        if($register_id < 1){
            wp_send_json_error(__("Register not found", "wc_point_of_sale"), 404);
        }
    ...
    public function find_products() {
        ...
        $words = array_values(array_filter($words)); // Remove empty elements if any.
        ...
        // Exit if search text is an empty string (i.e. string of spaces and/or tabs).
        if (empty($words)) {
        ...
        $query = "SELECT p.ID AS id,
            ...
            LEFT JOIN {$wpdb->postmeta} pm_stk ON p.ID = pm_stk.post_id AND pm_stk.meta_key = '_stock'
            LEFT JOIN {$wpdb->postmeta} pm_sts ON p.ID = pm_sts.post_id AND pm_sts.meta_key = '_stock_status'
            LEFT JOIN {$wpdb->postmeta} pm_vis ON p.ID = pm_vis.post_id AND pm_vis.meta_key = '_pos_visibility'
            WHERE p.post_type IN ('product', 'product_variation') 
                AND p.post_status = 'publish'
                AND (pm_vis.meta_value IS NULL OR pm_vis.meta_value NOT IN ('online')) AND (";

        $words_count = count($words);
        for ($i = 0; $i < $words_count; $i++) {
            $esc_word = esc_sql($words[$i]);
            $query .= "(p.post_title LIKE '%{$esc_word}%' OR pm_sku.meta_value LIKE '%{$esc_word}%')";
            $query .= ($i + 1) === $words_count ? ")" : " AND ";
        }

        // Exclude out of stock products if wc_pos_show_out_of_stock_products is not enabled.
        $show_out_of_stock = get_option('wc_pos_show_out_of_stock_products', 'no');
        if ($show_out_of_stock === 'no') {
            $query .= " AND pm_sts.meta_value NOT IN ('outofstock')";
        }
        ...
        if (!empty($products)) {
            foreach ($products as $p) {
                $item = array(
                    ...
                    'title' => $p->title,
                    'sku'   => $p->sku,
                    'stock' => is_int($p->stock) ? $p->stock : round($p->stock, 2),
                    'price' => wc_get_product($p->id)->get_price_html()
                );

                if ($p->type === 'product_variation') {
                    $item['id'] = $p->parent;
                    $item['vid'] = $p->id;
                }
    ...
    public function scan_product() {
        ...
        // Scanning field.
        $field = get_option('woocommerce_pos_register_scan_field', '_sku');
        $field = empty($field) ? '_sku' : $field;
        ...
        $query = "SELECT p.ID AS id,
                ...
                p.post_parent AS parent
            FROM {$wpdb->posts} p
            LEFT JOIN {$wpdb->postmeta} pm_fld ON p.ID = pm_fld.post_id AND pm_fld.meta_key = '{$field}'
            LEFT JOIN {$wpdb->postmeta} pm_sts ON p.ID = pm_sts.post_id AND pm_sts.meta_key = '_stock_status'
            LEFT JOIN {$wpdb->postmeta} pm_vis ON p.ID = pm_vis.post_id AND pm_vis.meta_key = '_pos_visibility'
            WHERE p.post_type IN ('product', 'product_variation')
                AND pm_fld.meta_value = '{$barcode}'
                AND p.post_status = 'publish'
                AND (pm_vis.meta_value IS NULL OR pm_vis.meta_value NOT IN ('online'))";
